...
#         modified_cv = call_openai_llm(prompt)
#         if not modified_cv:
...
'''
        prompt = (
            "You are an expert resume editor. Your goal is to maximize the CV's alignment with the job description while maintaining complete honesty and accuracy. "
            "CRITICAL RULES: 1) NEVER add skills, experiences, or qualifications that are not explicitly present in the original CV. 2) ONLY rephrase, reorder, and emphasize existing content. "
            "3) Use exact keywords and terminology from the job description when they match the candidate's actual experience. 4) Prioritize logical skill connections - if someone has 'Python programming', they likely have 'software development' skills. "
            "5) Emphasize transferable skills and relevant experience that align with job requirements. 6) Reorder sections to highlight most relevant experience first. "
            "7) Use action verbs and quantifiable achievements from the original CV. 8) Ensure the modified CV will score significantly higher in match-cv-description analysis. "
            "Output ONLY valid JSON using the provided schema. Do NOT include any explanations, markdown, or code blocks.\n\n"
            f"Schema:\n{schema}\n\nJob Description:\n{request_data.job_post_text}\n\nCV:\n{cv_text}"
        )
        
        json_str = call_openai_llm(prompt)
        if not json_str:
            raise HTTPException(status_code=500, detail="Failed to get response from LLM")
